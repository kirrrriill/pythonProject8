            ...
            if isinstance(section, NoteSection):
                for note in section.iter_notes():
                      ...
                      self._emitline('  %s %s\t%s' % (
                          ...
                          describe_note(note, machine=None)))
...
def process(file, stream=None):
    ...
    with open(file, "rb") as f:
        try:
            relf = ReadElf(f, stream or sys.stdout)
            if do_file_header:
                relf.display_file_header()
            if do_section_header:
                sections_data_list = relf.display_section_headers(
                        show_heading=not do_file_header)
            if do_program_header:
                relf.display_program_headers(
                        show_heading=not do_file_header)
            
                relf.display_dynamic_tags()

                relf.display_symbol_tables()

                relf.display_notes()

                relf.display_relocations()

                relf.display_version_info()

                relf.display_arch_specific()

                relf.display_hex_dump(1)

                relf.display_string_dump(1)

                relf.display_debug_dump(1)
